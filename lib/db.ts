...
export function getDb() {
  ...
  // after initialization completes successfully so a failure cannot leave
  // a partially initialized database behind.
  try {
    db.exec(`
    ...
    db.transaction(() => {
      ...
      const remCount = db.prepare("SELECT count(*) as count FROM reminders").get() as { count: number };
      if (remCount.count === 0) {
    ...
    try {
      ...
      // ignore close failure on a partially opened database
    }
